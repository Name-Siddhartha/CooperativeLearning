...
#################################################################################################
enhance_visual = concat(specific_V_g, input_V)
#enhance_visual = concat(enhance_visual, specific_V_h)

enhance_audio = concat(specific_A_g,  input_A)
#enhance_audio = concat(enhance_audio, specific_A_h)

enhance_textual = concat(specific_T_g, input_T)
#enhance_textual = concat(enhance_textual, specific_T_h)

#################################################################################################
distribution_v = gen_distribution(enhance_visual, feature_size)
distribution_a = gen_distribution(enhance_audio, feature_size)
distribution_t = gen_distribution(enhance_textual, feature_size)
    ...
    with tf.Session() as sess:
        with tf.device("/gpu:0"):
            ...
            for i in range(20000):
                ...
                _, _, _, _, l_v, l_a, l_t, pv, pa, pt, p, th_v, th_a, th_t = sess.run([train, train_v, train_a, train_t, loss_v, loss_a,
                                                                                      loss_t, preds_v, preds_a, preds_t, preds,
                                                                                       threshold_V_host, threshold_A_host, threshold_T_host], feed_dict={
                    input_V: v_input,
                    input_A: a_input,
                    input_T: t_input,
                    input_label: labels,
