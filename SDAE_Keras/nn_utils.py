...
def x_generator(X, batch_size, shuffle, seed=1337):
    ...
    # Creates batches of data from given input, given a batch size.
    # Returns dense representation of sparse input one batch a time.
    # X are input features, can be sparse or dense
    # batch_size is number of instances in each batch
    # If shuffle is True, shuffle input instances.
    # seed is seed value for randomization and replication

    number_of_batches = np.ceil(X.shape[0]/batch_size)
    # ceil function allows for creating last batch off remaining samples
    counter = 0
    sample_index = np.arange(X.shape[0])
